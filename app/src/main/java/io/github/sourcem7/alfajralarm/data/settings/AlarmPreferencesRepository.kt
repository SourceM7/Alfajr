...
class AlarmPreferencesRepository(private val context: Context) {
    val preferences: Flow<AlarmPreferences> = context.userPreferencesDataStore.data.map { stored ->
        AlarmPreferences(
    ...
    suspend fun updateLocation(location: FixedLocation) = context.userPreferencesDataStore.edit {
    ...
    suspend fun updateMethod(method: FajrMethod) = context.userPreferencesDataStore.edit { it[METHOD] = method.name }
